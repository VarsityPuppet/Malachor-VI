void SpawnObject(int nObjectType, string sTemplate, location lLocation, int bUseAppearAnimation=FALSE){
	object oSpawn = CreateObject(nObjectType, sTemplate, lLocation, bUseAppearAnimation);
}
...
void main(){
	...
	switch(param1){
		...
		case 1:		//Jump characters to right positions
			...
			//CreateObject(OBJECT_TYPE_PLACEABLE, "kreia_invis", Location(Vector(0.0, 1.5, 3.0), 0.0));
		...
		case 5:		//Atton decloacks, attacks Kreia.. Party sees that Atton is losing, they attack, 
			...
			DelayCommand(0.3, AssignCommand(oKreia, SetFacing(-90.0)));
				//Atton attacks
			DelayCommand(0.0, ChangeToStandardFaction(oAtton, 22));
			DelayCommand(0.0, ChangeToStandardFaction(oKreia, 22));
 			...
 			DelayCommand(0.7, AssignCommand(oAtton, ActionDoCommand(CutsceneAttack(oKreia, 114, ATTACK_RESULT_PARRIED, 0))));
 			//DelayCommand(0.9, AssignCommand(oKreia, ActionDoCommand(ActionWait(2.2))));
			//DelayCommand(1.7, AssignCommand(oAtton, ClearAllActions()));
			//DelayCommand(1.7, AssignCommand(oKreia, ClearAllActions()));
 			//DelayCommand(1.7, AssignCommand(oAtton, ActionDoCommand(ActionWait(1.0))));
			...
			DelayCommand(1.7, CancelCombat(oKreia));
			DelayCommand(1.7, ChangeToStandardFaction(oKreia, 5));
			DelayCommand(1.7, ChangeToStandardFaction(oAtton, 2));
			DelayCommand(1.7, SetLightsaberPowered(oKreia, TRUE, FALSE, FALSE));
			...
			DelayCommand(3.0, AssignCommand(oMira, SetFacingPoint(GetPosition(oKreia))));
			DelayCommand(3.0, AssignCommand(oVisas, SetFacingPoint(GetPosition(oKreia))));
			DelayCommand(3.0, AssignCommand(oHand, SetFacingPoint(GetPosition(oKreia))));
			DelayCommand(3.0, AssignCommand(oHanharr, SetFacingPoint(GetPosition(oKreia))));
			DelayCommand(3.0, AssignCommand(oDisciple, SetFacingPoint(GetPosition(oKreia))));
		...
		case 9:		//Rocket node
			...
			DelayCommand(3.0, AssignCommand(oMira, ActionPlayAnimation(10468, 1.0, 0.5)));
			DelayCommand(3.5, AssignCommand(oMira, ClearAllActions()));
			DelayCommand(3.5, AssignCommand(oMira, ActionCastSpellAtLocation(190, GetLocation(oKreia), 0 , TRUE, PROJECTILE_PATH_TYPE_DEFAULT, FALSE)));
			DelayCommand(4.0, SetDialogPlaceableCamera(iCam2));
			DelayCommand(4.2, SetGlobalFadeOut(0.0, 0.3, 1.0, 1.0, 1.0));
			DelayCommand(4.9, SetGlobalFadeIn(0.0, 0.3, 1.0, 1.0, 1.0));
			DelayCommand(4.1, SpawnObject(OBJECT_TYPE_PLACEABLE, "plc_steamc02", GetLocation(oKreia), TRUE));
			DelayCommand(4.1, SpawnObject(OBJECT_TYPE_PLACEABLE, "plc_steamc03", GetLocation(oKreia), TRUE));
			DelayCommand(4.1, SpawnObject(OBJECT_TYPE_PLACEABLE, "plc_steamc04", GetLocation(oKreia), TRUE));
			DelayCommand(4.3, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(3003, FALSE), GetLocation(oKreia)));
			DelayCommand(5.0, AssignCommand(oMira, ActionMoveToLocation(Location(Vector(2.20140, 1.95512, 2.25750), Deg(-2.29113)), TRUE)));
			DelayCommand(6.0, SetDialogPlaceableCamera(iCam3));
			DelayCommand(7.0, AssignCommand(GetObjectByTag("SteamContinual02"), SetIsDestroyable(TRUE, FALSE, FALSE)));
			DelayCommand(7.0, AssignCommand(GetObjectByTag("SteamContinual03"), SetIsDestroyable(TRUE, FALSE, FALSE)));
			DelayCommand(7.0, AssignCommand(GetObjectByTag("SteamContinual04"), SetIsDestroyable(TRUE, FALSE, FALSE)));
			...
			DelayCommand(8.3, DestroyObject(GetObjectByTag("SteamContinual02")));
			DelayCommand(8.3, DestroyObject(GetObjectByTag("SteamContinual03")));
			DelayCommand(8.3, DestroyObject(GetObjectByTag("SteamContinual04")));
		...
		case 15:	//Mira binoculars
			CreateObject(OBJECT_TYPE_PLACEABLE, "kreia_invis", Location(Vector(0.0, 0.0, 3.0), 0.0));
			AssignCommand(oKreia, ActionSwitchWeapons());
			SetLockOrientationInDialog(oMira, 0);
			SetLockOrientationInDialog(oHand, 0);
			SetLockOrientationInDialog(oDisciple, 0);
			SetLockOrientationInDialog(oAtton, 0);
			SetLockOrientationInDialog(oHanharr, 0);
			SetLockOrientationInDialog(oVisas, 0);
			//AssignCommand(oMira, JumpToLocation(Location(Vector(-0.35006, -72.93472, 0.79600), Deg(1.55308))));
			...
			DelayCommand(0.8, AssignCommand(oMira, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_mira_plan")))));
		...
		case 16:	//Sion leaves Kreia, party enters the core
			...
			DelayCommand(4.5, AssignCommand(oSion, ActionMoveToLocation(Location(Vector(9.05581, 8.33247, 0.47460), Deg(-2.36860)))));
			DelayCommand(7.0, DestroyObject(oSion));
			DelayCommand(1.6, AssignCommand(oHand, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_handisc_plan")))));
			DelayCommand(1.6, AssignCommand(oDisciple, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_handisc_plan")))));
			DelayCommand(1.0, AssignCommand(oVisas, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_visas_plan")))));
			DelayCommand(2.3, AssignCommand(oAtton, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_atton_plan")))));
			DelayCommand(0.8, AssignCommand(oMira, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_mira_plan")))));
			DelayCommand(0.5, AssignCommand(oHanharr, ActionMoveToLocation(GetLocation(GetObjectByTag("wp_hanharr_plan")))));
		break;
