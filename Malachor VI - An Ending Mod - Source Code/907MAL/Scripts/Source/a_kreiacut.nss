...
void main() {
	...
	switch (nParam1) {
		case 0:
			...
			AssignCommand(oDarthSion, ActionPlayAnimation(10035, 1.0, 100.0));
			break;
		case 1:
			{
				int int3 = 0;
		...
		case 5:
			...
			break;
		case 6:
			object oDarthSion = CreateObject(1, "n_darthsion", Location(Vector(0.18, -2.74217, 2.25750), 90.0), 0);
			AssignCommand(GetObjectByTag("DarthSion", 0), ClearAllActions());
			//AssignCommand(oDarthSion, ActionPlayAnimation(10019, 1.0, 1.0));
			DelayCommand(1.0, SetLightsaberPowered(GetObjectByTag("DarthSion", 0), 1, 1, 0));
		...
		case 8:
			...
			AssignCommand(GetObjectByTag("DarthSion", 0), ActionPlayAnimation(10460, 1.0, (-1.0)));
			DelayCommand(1.0, AssignCommand(GetObjectByTag("DarthSion", 0), ActionPlayAnimation(116, 1.0, (-1.0))));
			DelayCommand(3.5, ActionResumeConversation());*/
			break;
		case 9:
			ApplyEffectToObject(1, EffectChoke(), GetObjectByTag("DarthSion", 0), 15.0);
			break;
		case 10:

			SetGlobalNumber("907MAL_CUTSCENE", 3);
			ExecuteScript("a_cleanup", GetFirstPC(), 0xFFFFFFFF);
