...
void DoPlanetChange()
{
    ...
    else if( (GetGlobalNumber("003EBO_BACKGROUND") == 9)  &&
               (GetGlobalNumber("900MAL_Open") == 1)) 
    {// first time to Malachor V
        SetGlobalNumber("900MAL_Open", 2);
        DoFirst901();
    }
    else
    {
...
void DoFirst901()
{
  ...
  location wp900visas = Location(Vector(51.07794, 76.45095, 0.15298), 90.00);
  location wp900mira = Location(Vector(51.68631, 79.24873, 0.27415), 90.00);
  location wp900disc = Location(Vector(54.28363, 77.34167, -0.33646), 90.00);
  location wp900hand = Location(Vector(53.67108, 79.23895, -0.06775), 90.00);
  location wp900mand = Location(Vector(51.90546, 78.51255, 0.09898), 90.00);
    ...
    //AssignCommand(oAtton, ActionJumpToLocation(GetLocation(GetObjectByTag("WP_301_atton"))));
    AssignCommand(oMand, ActionJumpToLocation(wp900mand ));
    AssignCommand(oMira, ActionJumpToLocation(wp900mira ));
    AssignCommand(oDisc, ActionJumpToLocation(wp900disc ));
    AssignCommand(oMaid, ActionJumpToLocation(wp900hand));
    AssignCommand(oVisas, ActionJumpToLocation(wp900visas));
        PlayMovieQueue();
    SignalEvent(GetArea(oPlayer), EventUserDefined(1));
    AssignCommand(oPlayer, ClearAllActions());
    AssignCommand(oPlayer, ActionJumpToLocation(GetLocation(GetObjectByTag("WP_301_player"))));
    ...
    //object holo = CreateObject(OBJECT_TYPE_PLACEABLE, "fake_holo", GetLocation(GetObjectByTag("hologram"))); 

    //DelayCommand(1.5, AssignCommand(holo, ActionStartConversation(oPlayer, "003mal", 0, 0, 1, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0)));
}
